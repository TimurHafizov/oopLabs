...
namespace lab14
{
    ...
    public class MyQueue<T> : IList<T>, ICloneable, IComparable<T>
    {
        ...
        {
            if (item == null) throw new ArgumentNullException("Поиск пустого элемента карается по закону, но это не точно..");
        ...
        {
            if (index < 0 || index > Count) throw new Exception($"Ууу, давайте не усложнять: новый элемент должен быть с индексом меньше {Count}..");
            if (index == 0)
            {
                MyQueue<T> cur = new MyQueue<T>();
                cur.data = item;
